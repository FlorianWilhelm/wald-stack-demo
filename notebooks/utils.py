...
def get_dbt_credentials(profile_name: str, target_name: str) -> Credentials:
    ...
    creds = Profile._credentials_from_profile(
        raw_profiles[profile_name]["outputs"][target_name], profile_name, target_name
    )
    return creds


def get_snowflake_cursor_from_dbt(
    profile_name: str, target_name: str
) -> SnowflakeConnection:
...
def get_snowflake_dbt_adapter(profile_name: str, target_name: str) -> SnowflakeAdapter:
    ...
    Replicates to some degree what dbt does internally. Meant for debugging only as
    this is rather a hack!

    ToDo: Generalize this by using dbt's functionality to retrieve the right adapter
     from the `type` field in the profile instead of hard-coding SnowflakeAdapter.
    """
    creds = get_dbt_credentials(profile_name, target_name)
